...
template <> struct real_type<float>
{
    typedef double type;
};
template <> struct real_type<double>
{
    typedef double type;
};
template <> struct real_type<long double>
{
...
inline bool string_to_type_converter(const Iterator begin, const Iterator end,
                                     T& t)
{
...
template <typename Iterator, typename T>
inline bool string_to_type_converter(const std::pair<Iterator,Iterator>& range,
                                     T& t)
{
...
{
    return string_to_type_converter<const char*,T>(to_ptr(s), to_ptr(s) + s.size(),
            t);
}
                                      ...
                                      const std::string& delimiter = "")
{
    ...
    std::size_t count = 0;
    typename std::multiset<T,Comparator,Allocator>::const_iterator itr =
        multiset.begin();
    typename std::multiset<T,Comparator,Allocator>::const_iterator end =
        multiset.end();
...
template <typename T,
          typename Allocator,
          template <typename,typename> class Sequence>
inline adapter<typename Sequence<T,Allocator>::iterator> type(
    const Sequence<T,Allocator>& seq)
{
...
private:
    ...
    multiple_delimiter_predicate& operator=(const multiple_delimiter_predicate<T>&
                                            mdp);
...
template <typename Allocator,
          template <typename,typename> class Sequence>
inline details::index_remover_impl<Allocator,Sequence> index_remover(
    const Sequence<std::size_t,Allocator>& sequence)
{
...
inline std::size_t remove_inplace(Predicate predicate,
                                  Sequence<T,Allocator>& sequence)
{
    const std::size_t removal_count = remove_inplace(predicate,sequence.begin(),
                                      sequence.end());
...
{
    const std::size_t removal_count = remove_inplace(
                                          single_delimiter_predicate<std::string::value_type>(c),
                                        ...
                                        std::string& s)
{
    ...
    const std::size_t removal_count = remove_consecutives_inplace(
                                          single_delimiter_predicate<std::string::value_type>(c),
                                          const_cast<char*>(to_ptr(s)),
                                          const_cast<char*>(to_ptr(s) + s.size()));
...
inline void remove_consecutives_inplace(const std::string& rem_chars,
                                        std::string& s)
{
    if (s.empty())
    {
    ...
    const std::size_t removal_count = remove_consecutives_inplace(
                                          multiple_char_delimiter_predicate(rem_chars),
                                          const_cast<char*>(to_ptr(s)),
                                          const_cast<char*>(to_ptr(s) + s.size()));
...
inline void remove_consecutives_inplace(const char* rem_chars, std::string& s)
{
    ...
    const std::size_t removal_count = remove_consecutives_inplace(
                                          multiple_char_delimiter_predicate(
                                                  rem_chars,
...
{
    const std::size_t removal_count = remove_consecutives_inplace(sequence.begin(),
                                      sequence.end());
    sequence.resize(sequence.size() - removal_count);
}
...
{
    std::size_t removal_count = remove_consecutives_inplace(const_cast<char*>
                                (to_ptr(s)),
                                const_cast<char*>(to_ptr(s) + s.size()));
...
inline void remove_trailing(const std::string::value_type c, std::string& s)
{
    ...
    const std::size_t removal_count = remove_trailing(
                                          single_delimiter_predicate<std::string::value_type>(c),
                                          const_cast<char*>(to_ptr(s)),
                                          const_cast<char*>(to_ptr(s) + s.size()));
...
inline void remove_leading(const std::string::value_type c, std::string& s)
{
    ...
    const std::size_t removal_count = remove_leading(
                                          single_delimiter_predicate<std::string::value_type>(c),
                                          const_cast<char*>(to_ptr(s)),
                                          const_cast<char*>(to_ptr(s) + s.size()));
...
inline void remove_leading_trailing(const std::string& rem_chars,
                                    std::string& s)
{
...
void remove_empty_strings(std::multiset<std::string,Comparator,Allocator>& set)
{
    ...
    typename std::multiset<std::string,Comparator,Allocator>::iterator itr =
        set.begin();
...
template <typename InputIterator, typename OutputIterator>
inline std::size_t replace_pattern(const InputIterator s_begin,
                                   const InputIterator s_end, // Input
                       ...
                       const typename std::iterator_traits<Iterator>::value_type& match_zero_or_one)
{
    ...
    while ((p_itr != pattern_end) && (d_itr != data_end))
    {
        ...
        {
            while ((p_itr != pattern_end) && (*p_itr == match_zero_or_more ||
                                              *p_itr == match_zero_or_one))
            {
                ++p_itr;
            }
                  ...
                  const std::string& str)
{
    ...
    */
    return details::match_impl<details::cs_match>(to_ptr(wild_card),
            to_ptr(wild_card) + wild_card.size(),
...
template <typename Iterator>
inline Iterator imatch(const std::string& s, const Iterator begin,
                       const Iterator end)
{
...
template <typename Allocator,
          template <typename,typename> class Sequence>
inline bool imatch(const std::string& s,
...
{
    return find_all(pattern_begin, pattern_end, begin, end,
...
inline std::size_t ifind(const std::string& pattern, const std::string& data)
{
    ...
    const char* result_itr = std::search(to_ptr(data),
                                         to_ptr(data)    + data   .size(),
    ...
    public:
        ...
        explicit inline tokenizer_iterator(const iterator begin,
            ...
            : predicate_(predicate),
              ...
              compress_delimiters_(tokenize_options::perform_compress_delimiters(
                                       tokenize_option)),
              include_1st_delimiter_(tokenize_options::perform_include_1st_delimiter(
                                         tokenize_option)),
              include_all_delimiters_(tokenize_options::perform_include_all_delimiters(
                                          tokenize_option)),
...
struct tokenizer
{
    ...
    typedef strtk::tokenizer<string_iterator_type,multiple_char_delimiter_predicate>
    md_type;
...
class range_to_type_back_inserter_iterator : public
...
public:
    ...
    range_to_type_back_inserter_iterator(const range_to_type_back_inserter_iterator&
                                         itr)
        : sequence_(itr.sequence_)
    {}

    inline range_to_type_back_inserter_iterator& operator=(const
            range_to_type_back_inserter_iterator& itr)
    {
        if (this != &itr)
        {
...
template <typename Sequence>
inline range_to_type_back_inserter_iterator<Sequence>
range_to_type_back_inserter(Sequence& sequence)
{
    return (range_to_type_back_inserter_iterator<Sequence>(sequence));
}

template <typename Set>
class range_to_type_inserter_iterator : public
    std::iterator<std::output_iterator_tag,
    void,
    void,
    void,
...
public:
    ...
    inline range_to_type_inserter_iterator& operator=(const
            range_to_type_inserter_iterator& itr)
    {
        if (this != &itr)
        {
            this->set_ = itr.set_;
...
class range_to_type_push_inserter_iterator : public
    std::iterator<std::output_iterator_tag,
    void,
    void,
    void,
...
public:
    ...
    range_to_type_push_inserter_iterator(const range_to_type_push_inserter_iterator&
                                         itr)
        : container_(itr.container_)
    {}

    inline range_to_type_push_inserter_iterator& operator=(const
            range_to_type_push_inserter_iterator& itr)
    {
        if (this != &itr)
        {
...
template <typename Container>
inline range_to_type_push_inserter_iterator<Container>
range_to_type_push_inserter(Container& container)
{
    return (range_to_type_push_inserter_iterator<Container>(container));
}

template <typename Sequence>
class back_inserter_with_valuetype_iterator : public
    std::iterator<std::output_iterator_tag,
...
public:
    ...
    back_inserter_with_valuetype_iterator(const
                                          back_inserter_with_valuetype_iterator& itr)
        : sequence_(itr.sequence_)
    {}

    inline back_inserter_with_valuetype_iterator& operator=(const
            back_inserter_with_valuetype_iterator& itr)
    {
        if (this != &itr)
        {
    ...
    inline back_inserter_with_valuetype_iterator& operator=(const typename
            Sequence::value_type& v)
    {
...
template <typename Sequence>
inline back_inserter_with_valuetype_iterator<Sequence>
back_inserter_with_valuetype(Sequence& sequence_)
{
    return (back_inserter_with_valuetype_iterator<Sequence>(sequence_));
}

template <typename Set>
class inserter_with_valuetype_iterator : public
    std::iterator<std::output_iterator_tag,
...
public:
    ...
    inline inserter_with_valuetype_iterator& operator=(const
            inserter_with_valuetype_iterator& itr)
    {
        if (this != &itr)
        {
            this->set_ = itr.set_;
    ...
    inline inserter_with_valuetype_iterator& operator=(const typename
            Set::value_type& v)
    {
        set_.insert(v);
        return (*this);
...
public:
    ...
    inline push_inserter_iterator<Container>& operator=(typename
...
template <typename T>
class range_to_ptr_type_iterator : public
    std::iterator<std::output_iterator_tag,
    void,
    void,
    void,
...
public:
    ...
    explicit inline range_to_ptr_type_iterator(T* pointer,
    ...
    inline range_to_ptr_type_iterator& operator=(const range_to_ptr_type_iterator&
            itr)
    {
        if (this != &itr)
        {
    ...
    template <typename Iterator>
    inline range_to_ptr_type_iterator& operator=(const std::pair<Iterator,Iterator>&
            r)
    {
...
template <typename T>
class counting_back_inserter_iterator : public
    std::iterator<std::output_iterator_tag,
    T,
    void,
    void,
...
public:
    ...
    inline counting_back_inserter_iterator& operator=(const
            counting_back_inserter_iterator& itr)
    {
        if (this != &itr)
        {
...
template <typename T>
inline counting_back_inserter_iterator<T> counting_back_inserter(
    std::size_t& counter_)
{
...
template <typename Function>
class functional_inserter_iterator : public
    std::iterator<std::output_iterator_tag,
    void,
    void,
    void,
...
public:
    ...
    inline functional_inserter_iterator& operator=(const
            functional_inserter_iterator& itr)
    {
        if (this != &itr)
        {
...
template <typename Function>
inline functional_inserter_iterator<Function> functional_inserter(
    Function function)
{
...
template <typename DelimiterPredicate,
          typename Iterator,
          typename OutputIterator>
inline std::size_t split_compress_delimiters(const DelimiterPredicate&
        delimiter,
        const Iterator begin,
        const Iterator end,
        OutputIterator out)
                         ...
                         const split_options::type split_option = split_options::default_mode)
{
    ...
    const bool compress_delimiters    = split_options::perform_compress_delimiters(
                                            split_option);
    const bool include_1st_delimiter  =
        split_options::perform_include_1st_delimiter(split_option);
    const bool include_all_delimiters = (!include_1st_delimiter) &&
                                        split_options::perform_include_all_delimiters(split_option);
    const bool include_delimiters     = include_1st_delimiter ||
                                        include_all_delimiters;
...
{
    if (1 == delimiters.size())
        return split_n(single_delimiter_predicate<std::string::value_type>
                       (delimiters[0]),
                       to_ptr(str), to_ptr(str) + str.size(),
                       token_count,
                       out,
...
uri_expression     ("((https?|ftp)\\://((\\[?(\\d{1,3}\\.){3}\\d{1,3}\\]?)|(([-a-zA-Z0-9]+\\.)+[a-zA-Z]{2,4}))(\\:\\d+)?(/[-a-zA-Z0-9._?,+&amp;%$#=~\\\\]+)*/?)");
static const std::string
email_expression   ("([\\w\\-\\.]+)@((\\[([0-9]{1,3}\\.){3}[0-9]{1,3}\\])|(([\\w\\-]+\\.)+)([a-zA-Z]{2,4}))");
static const std::string
ip_expression      ("(([0-2]*[0-9]+[0-9]+)\\.([0-2]*[0-9]+[0-9]+)\\.([0-2]*[0-9]+[0-9]+)\\.([0-2]*[0-9]+[0-9]+))");
static const std::string
ieee754_expression ("([-+]?((\\.[0-9]+|[0-9]+\\.[0-9]+)([eE][-+][0-9]+)?|[0-9]+))");
...
inline offset_predicate<12> offsets(const int&  v1, const int&  v2,
                                    const int&  v3,
...
inline offset_predicate<11> offsets(const int&  v1, const int&  v2,
                                    const int& v3,
                                    const int&  v4, const int&  v5, const int& v6,
                                    const int&  v7, const int&  v8, const int& v9,
...
inline offset_predicate<2> offsets(const int& v1, const int& v2,
                                   const bool& rotate = false)
{
                                   ...
                                   OutputIterator out)
{
    ...
    if (0 == (length = std::distance(begin,end)))
    {
        return 0;
    }
...
template <typename InputIterator>
inline std::size_t count_consecutive_duplicates(const InputIterator begin,
        const InputIterator end)
{
...
template <typename T,
          typename Comparator,
          typename Allocator>
inline void min_max_of_cont(const std::multiset<T,Comparator,Allocator>&
                            multiset,
                            T& min_value,
                            T& max_value)
{
...
inline void lexicographically_canonicalize(Iterator begin, Iterator end)
{
    ...
    for (Iterator itr = begin; itr != end; ++itr)
    {
        if (*itr == smallest)
        {
            itr_list.push_back(std::make_pair(itr,itr));
        }
    }
...
inline const char* first_non_repeated_char(const char* begin, const char* end)
{
    ...
    static const unsigned long long int not_yet_encountered =
...
inline const unsigned char* first_non_repeated_char(const unsigned char* begin,
        const unsigned char* end)
{
    char* b = reinterpret_cast<char*>(const_cast<unsigned char*>(begin));
    char* e = reinterpret_cast<char*>(const_cast<unsigned char*>(end));

    return const_cast<const unsigned char*>(reinterpret_cast<unsigned char*>
                                            (const_cast<char*>(first_non_repeated_char(b,e))));
}
...
inline std::size_t first_non_repeated_char(const std::string& str)
{
    ...
    const char* itr = first_non_repeated_char(to_ptr(str),
                      to_ptr(str) + str.size());
...
inline void convert_bin_to_hex(const unsigned char* begin,
...
inline void convert_bin_to_hex(const std::pair<unsigned char*,unsigned char*>&
                               r, unsigned char* out)
{
    convert_bin_to_hex(r.first,r.second,out);
}

inline void convert_bin_to_hex(const
                               std::pair<const unsigned char*,const unsigned char*>& r, unsigned char* out)
{
    convert_bin_to_hex(r.first,r.second,out);
}

inline void convert_bin_to_hex(const std::pair<const char*,const char*>& r,
                               char* out)
{
    convert_bin_to_hex(r.first,r.second,out);
}

inline void convert_bin_to_hex(const std::string& binary_data,
                               std::string& output)
{
...
inline std::size_t convert_bin_to_base64(const unsigned char* begin,
        const unsigned char* end, unsigned char* out)
{
    static const unsigned char bin_to_base64 [] =
...
inline std::size_t convert_bin_to_base64(const char* begin, const char* end,
        char* out)
{
...
inline void convert_bin_to_base64(const std::string& binary_data,
                                  std::string& output)
{
...
inline std::size_t convert_base64_to_bin(const unsigned char* begin,
        const unsigned char* end, unsigned char* out)
{
...
inline std::size_t convert_base64_to_bin(const char* begin, const char* end,
        char* out)
{
...
inline void convert_base64_to_bin(const std::string& binary_data,
                                  std::string& output)
{
...
{
    convert_to_printable_chars(reinterpret_cast<unsigned char*>(const_cast<char*>
                               (to_ptr(str))),
...
{
    convert_to_uppercase(reinterpret_cast<unsigned char*>(const_cast<char*>(to_ptr(
                             str))),
                         reinterpret_cast<unsigned char*>(const_cast<char*>(to_ptr(str) + str.size())));
}
...
{
    convert_to_lowercase(reinterpret_cast<unsigned char*>(const_cast<char*>(to_ptr(
                             str))),
                         reinterpret_cast<unsigned char*>(const_cast<char*>(to_ptr(str) + str.size())));
}
...
inline bool twoway_bitwise_interleave(const unsigned char* begin1,
...
template<> struct interleave_ary<sizeof(unsigned short)>
{
    typedef unsigned short         type;
};
template<> struct interleave_ary<sizeof(unsigned int  )>
{
    typedef unsigned int           type;
};
template<> struct interleave_ary<sizeof(unsigned long long int)>
{
    typedef unsigned long long int type;
};

template <std::size_t n>
inline void create_nway_interleave_table(typename interleave_ary<n>::type
        table[256])
{
...
inline std::size_t high_bit_count(const unsigned char* begin,
                                  const unsigned char* end)
{
    std::size_t count = 0;
    const unsigned char* itr = begin;
...
inline std::size_t hamming_distance(const unsigned char* begin1,
                                    const unsigned char* end1,
                                    const unsigned char* begin2, const unsigned char* end2)
{
    if (std::distance(begin1,end1) != std::distance(begin2,end2))
    ...
    while (end1 != itr1)
    {
        distance += high_bit_count(static_cast<unsigned char>(((*itr1++) ^
                                   (*itr2++)) & 0xFF));
    }
...
inline std::size_t hamming_distance(const std::string& str1,
                                    const std::string& str2)
{
...
template <typename Iterator>
inline std::size_t hamming_distance_elementwise(const Iterator begin1,
        const Iterator end1,
...
inline std::size_t hamming_distance_elementwise(const std::string& str1,
        const std::string& str2)
{
...
private:
    ...
    struct store
    {
        ...
        inline bool remove_row(const std::size_t& row)
        {
            ...
            token_list_t::iterator remove_end = token_list.begin() + r.first +
                                                number_of_tokens;
        ...
        inline bool remove_row_range(const std::size_t& r0, const std::size_t& r1)
        {
            ...
            token_list_t::iterator remove_end   = token_list.begin() + rr0.first +
                                                  number_of_tokens;
        ...
        {
            static const range_t null_range_ = range_t(
    ...
    struct row_processor
    {
        ...
        row_processor<DelimiterPredicate> operator=(const
                row_processor<DelimiterPredicate>&);
...
public:
                             ...
                             index_t upper_bound = std::numeric_limits<index_t>::max()) const
    {
        ...
        {
            return row_range_t(std::numeric_limits<index_t>::max(),
    ...
    public:
        ...
        {
            if (!process(*(begin_ + col0),t0))
            {
                return false;
            }

            if (!process(*(begin_ + col1),t1))
            {
                return false;
            }

            if (!process(*(begin_ + col2),t2))
            {
                return false;
            }

            if (!process(*(begin_ + col3),t3))
            {
                return false;
            }

            if (!process(*(begin_ + col4),t4))
            {
                return false;
            }

            if (!process(*(begin_ + col5),t5))
            {
                return false;
            }

            if (!process(*(begin_ + col6),t6))
            {
                return false;
            }

            if (!process(*(begin_ + col7),t7))
            {
                return false;
            }

            if (!process(*(begin_ + col8),t8))
            {
                return false;
            }

            if (!process(*(begin_ + col9),t9))
            {
                return false;
            }
        ...
        inline bool parse(T0& t0, T1& t1, T2& t2, T3& t3,
                          ...
                          T8& t8, T9& t9) const
        {
            if (!process(*(begin_ + 0),t0))
            {
                return false;
            }

            if (!process(*(begin_ + 1),t1))
            {
                return false;
            }

            if (!process(*(begin_ + 2),t2))
            {
                return false;
            }

            if (!process(*(begin_ + 3),t3))
            {
                return false;
            }

            if (!process(*(begin_ + 4),t4))
            {
                return false;
            }

            if (!process(*(begin_ + 5),t5))
            {
                return false;
            }

            if (!process(*(begin_ + 6),t6))
            {
                return false;
            }

            if (!process(*(begin_ + 7),t7))
            {
                return false;
            }

            if (!process(*(begin_ + 8),t8))
            {
                return false;
            }

            if (!process(*(begin_ + 9),t9))
            {
                return false;
            }
        ...
        template <typename T,
                  typename Container,
                  typename Comparator>
        inline bool parse(std::priority_queue<T,Container,Comparator>& priority_queue)
        const
        {
        ...
        template <typename T,
                  typename Allocator,
                  template <typename,typename> class Sequence>
        inline std::size_t parse_n(const std::size_t& n,
                                   Sequence<T,Allocator>& sequence) const
        {
            if (0 == n)
            {
        ...
        template <typename T,
                  typename Container,
                  typename Comparator>
        inline void parse_checked(std::priority_queue<T,Container,Comparator>&
    ...
    token_grid(const std::string& input_buffer,
               ...
               const token_grid::options& options)
        : file_name_(""),
          buffer_(reinterpret_cast<unsigned char*>(const_cast<char*>(to_ptr(
                      input_buffer)))),
          buffer_size_(input_buffer_size),
          min_column_count_(0),
          max_column_count_(0),
                                       ...
                                       OutputIterator out) const
    {
        ...
        for (std::size_t i = row_range.first; i < row_range.second; ++i)
        {
            ...
            if (index < dsv_index_.token_count(row))
            {
                ...
                process_token_checked(*(dsv_index_.token_list.begin() + (row.first + index)),
                                      out);
            }
        }
                               ...
                               OutputIterator3 out3) const
    {
        ...
        std::size_t max_index = std::max(std::max(index0,index1),std::max(index2,
                                         index3));
                               ...
                               OutputIterator4 out4) const
    {
        ...
        std::size_t max_index = std::max(index4,std::max(std::max(index0,index1),
                                         std::max(index2,index3)));
    ...
    template <typename Predicate>
    inline std::size_t remove_token_if(const row_range_t& row_range,
                                       Predicate predicate)
    {
        if (row_range_invalid(row_range))
        {
            ...
            const std::size_t& max_column_count)
    {
        ...
        remove_row_if(insufficient_number_of_minmax_columns(min_column_count,
                      max_column_count));
    ...
    template <typename Allocator,
              template <typename,typename> class Sequence>
    inline void get_column_widths(
        Sequence<std::pair<std::size_t,std::size_t>,Allocator>& columns)
    {
        for (std::size_t c = 0; c < max_column_count(); ++c)
        {
                         ...
                         std::string& result)
    {
        ...
        token_list_t::const_iterator itr = dsv_index_.token_list.begin() + r.first;
        token_list_t::const_iterator end = dsv_index_.token_list.begin() + r.second +
                                           (row < (dsv_index_.row_index.size() - 1) ? 1 : 0);

        result.reserve(delimiter.size() * dsv_index_.token_count(r) + std::distance(
                           itr->first,end->second));
...
private:
    ...
    {
        insufficient_number_of_minmax_columns(const std::size_t& min_col,
    ...
    inline bool load_buffer_from_file()
    {
        ...
        stream.read(reinterpret_cast<char*>(buffer_),
    ...
    template <typename OutputIterator>
    inline void process_token_checked(const range_t& range,
                                      OutputIterator out) const
    {
        typedef typename std::iterator_traits<OutputIterator>::value_type output_type;
...
template <typename T>
inline bool convert_string_range(const
                                 std::pair<std::string::const_iterator,std::string::const_iterator>& range, T& t)
{
    if (range.first == range.second)
    {
        return false;
...
public:
    ...
    inline void operator() (const std::pair<Iterator,Iterator>& range) const
    {
        for (const std::string* itr = begin_; end_ != itr; ++itr)
        {
            if (
                (case_insensitive_ &&
                 (imatch(to_ptr((*itr)), to_ptr((*itr)) + (*itr).size(), range.first,
                         range.second))) ||
...
struct size_equal_to
{
    ...
    template <typename T,
              typename Comparator,
              typename Allocator>
    inline bool operator()(const std::multiset<T,Comparator,Allocator>& multiset)
    const
    {
...
struct size_less_than
{
    ...
    inline bool operator()(const Iterator begin, const Iterator end) const
    {
        return std::distance(begin,
                             end) < static_cast<typename std::iterator_traits<Iterator>::difference_type>
               (length);
    }
    ...
    inline bool operator()(const std::pair<Iterator,Iterator>& range) const
    {
        return std::distance(range.first,
                             range.second) <
               static_cast<typename std::iterator_traits<Iterator>::difference_type>(length);
    }
...
private:
    ...
    protected:
        ...
        bool add_impl(const itr_type begin, const itr_type end) const
        {
            ...
            if (!string_to_type_converter(begin, end, t))
            {
                return false;
            }
...
public:
    ...
    container_adder(std::vector<T,Allocator>& vec)
        : container_adder_base_(new(buffer)sequence_adder_impl<T,Allocator,std::vector>
                                (vec))
    {}

    template <typename T, typename Allocator>
    container_adder(std::deque<T,Allocator>& deq)
        : container_adder_base_(new(buffer)sequence_adder_impl<T,Allocator,std::deque>
                                (deq))
    {}

    template <typename T, typename Allocator>
    container_adder(std::list<T,Allocator>& list)
        : container_adder_base_(new(buffer)sequence_adder_impl<T,Allocator,std::list>
                                (list))
    {}

    template <typename T, typename Comparator, typename Allocator>
    container_adder(std::set<T,Comparator,Allocator>& set)
        : container_adder_base_(new(buffer)
                                set_adder_impl<T,Comparator,Allocator,std::set>(set))
    {}

    template <typename T, typename Comparator, typename Allocator>
    container_adder(std::multiset<T,Comparator,Allocator>& multiset)
        : container_adder_base_(new(buffer)
                                set_adder_impl<T,Comparator,Allocator,std::multiset>(multiset))
    {}
    ...
    container_adder(std::queue<T,Container>& queue)
        : container_adder_base_(new(buffer)
                                stack_queue_adder_impl<T,Container,std::queue>(queue))
    {}

    template <typename T, typename Container>
    container_adder(std::stack<T,Container>& stack)
        : container_adder_base_(new(buffer)
                                stack_queue_adder_impl<T,Container,std::stack>(stack))
    {}
...
template <typename T>
struct ca_type <T,details::yes_t>
{
    typedef  details::container_adder type;
};
                  ...
                  T13& t13, T14& t14, T15& t15, T16& t16)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 ...
                 typename details::ca_type<T16, typename details::is_stl_container<T12>::result_t>::type(
                     t16));
}
                  ...
                  T13& t13, T14& t14, T15& t15)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 ...
                 typename details::ca_type<T15, typename details::is_stl_container<T12>::result_t>::type(
                     t15));
}
                  ...
                  T13& t13, T14& t14)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 ...
                 typename details::ca_type<T14, typename details::is_stl_container<T12>::result_t>::type(
                     t14));
}
                  ...
                  T13& t13)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 ...
                 typename details::ca_type<T13, typename details::is_stl_container<T12>::result_t>::type(
                     t13));
}
                  ...
                  T9& t9, T10& t10, T11& t11, T12& t12)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 ...
                 typename details::ca_type<T12, typename details::is_stl_container<T12>::result_t>::type(
                     t12));
}
                  ...
                  T9& t9, T10& t10, T11& t11)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 ...
                 typename details::ca_type<T11, typename details::is_stl_container<T11>::result_t>::type(
                     t11));
}
                  ...
                  T9& t9, T10& t10)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 ...
                 typename details::ca_type<T10, typename details::is_stl_container<T10>::result_t>::type(
                     t10));
}
                  ...
                  T9& t9)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 ...
                 typename details::ca_type<T9, typename details::is_stl_container<T9>::result_t>::type(
                     t9));
}
                  ...
                  T5& t5, T6& t6, T7& t7, T8& t8)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 ...
                 typename details::ca_type<T8, typename details::is_stl_container<T8>::result_t>::type(
                     t8));
}
                  ...
                  T5& t5, T6& t6, T7& t7)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 delimiters,
                 t1,t2,t3,t4,t5,t6,
                 typename details::ca_type<T7, typename details::is_stl_container<T7>::result_t>::type(
                     t7));
}
                  ...
                  T5& t5, T6& t6)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 delimiters,
                 t1,t2,t3,t4,t5,
                 typename details::ca_type<T6,typename details::is_stl_container<T6>::result_t>::type(
                     t6));
}
                  ...
                  T5& t5)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 delimiters,
                 t1,t2,t3,t4,
                 typename details::ca_type<T5, typename details::is_stl_container<T5>::result_t>::type(
                     t5));
}
                  ...
                  T1& t1, T2& t2, T3& t3, T4& t4)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 delimiters,
                 t1,t2,t3,
                 typename details::ca_type<T4, typename details::is_stl_container<T4>::result_t>::type(
                     t4));
}
                  ...
                  T1& t1, T2& t2, T3& t3)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 delimiters,
                 t1,t2,
                 typename details::ca_type<T3, typename details::is_stl_container<T3>::result_t>::type(
                     t3));
}
                  ...
                  T1& t1, T2& t2)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 delimiters,
                 t1,
                 typename details::ca_type<T2, typename details::is_stl_container<T2>::result_t>::type(
                     t2));
}
                  ...
                  T& t)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(),
                 delimiters,
                 typename details::ca_type<T,typename details::is_stl_container<T>::result_t>::type(
                     t));
}
...
template <typename T,
          typename Comparator,
          typename Allocator>
inline void iota(std::set<T,Comparator,Allocator>& set, std::size_t count,
                 T value)
{
    while (count)
    {
...
template <typename T,
          typename Comparator,
          typename Allocator>
inline void iota(std::multiset<T,Comparator,Allocator>& multiset,
...
public:
    ...
    translation_table(const std::string& itable, const std::string& otable)
    {
        ...
        {
            table_[static_cast<unsigned int>(itable[i])] = static_cast<unsigned char>
                    (otable[i]);
        }
    }
...
inline std::string translate(const translation_table& trans_table,
                             const std::string& s)
{
    std::string result = s;
...
inline void translate_inplace(const translation_table& trans_table,
                              std::string& s)
{
                                 ...
                                 unsigned int seed = magic_seed)
{
    ...
    boost::uniform_int<unsigned int> dist(std::numeric_limits<unsigned int>::min(),
                                        ...
                                        rand_int_type_tag)
{
    ...
    variate_generator<RandomNumberGenerator&,uniform_int<T> > rnd(rng,
            uniform_int<T>(min,max));
                                        ...
                                        rand_real_type_tag)
{
    // Note: The implied range will be: [min,max)
    using namespace boost;
    variate_generator<RandomNumberGenerator&, uniform_real<T> > rnd(rng,
            uniform_real<T>(min,max));
...
private:
    ...
    typedef boost::variate_generator<rng_type, boost::uniform_real<double> >
    variate_type;
                                   ...
                                   const std::size_t& pregen = 0)
{
    ...
    generate_random_values_impl(count,min,max,std::back_inserter(sequence),rng,
                                type);
}
                               ...
                               const std::size_t& pregen = 0)
{
    ...
    boost::variate_generator<boost::mt19937&, boost::uniform_real<double> > rnd(rng,
            dist);
                               ...
                               OutputIterator out)
{
    ...
    if ((size < set_size) || (rng. min() < 0.0) || (rng.max() > 1.0))
    {
        return false;
    }
...
{
template <typename Iterator>
inline Iterator prev(Iterator itr,
...
template <typename Iterator>
inline Iterator next(Iterator itr,
                     typename std::iterator_traits<Iterator>::difference_type n = 1)
{
    std::advance(itr, n);
    return itr;
...
template <typename Iterator>
bool next_combination(const Iterator first, const Iterator k,
                      const Iterator last)
{
    if (
        (first == last) ||
...
template <typename T,
          typename Allocator,
          template <typename,typename> class Sequence>
inline bool next_combination(Sequence<T,Allocator>& sequence,
                             const std::size_t& size)
{
    return next_combination(sequence.begin(), sequence.begin() + size,
                            sequence.end());
}

template <typename Iterator, typename Function>
inline void for_each_permutation(Iterator begin, Iterator end,
                                 Function function)
{
    do
    {
...
template <typename Iterator, typename Function>
inline bool for_each_permutation_conditional(Iterator begin, Iterator end,
        Function function)
{
    do
    {
...
inline void combine_discontinuous(Iterator first1, Iterator last1,
                                  typename std::iterator_traits<Iterator>::difference_type d1,
                                  Iterator first2, Iterator last2,
                                  typename std::iterator_traits<Iterator>::difference_type d2,
                                  Function& f,
...
template <typename Iterator, class Function>
inline bool combine_discontinuous_conditional(Iterator first1, Iterator last1,
        typename std::iterator_traits<Iterator>::difference_type d1,
        Iterator first2, Iterator last2,
        typename std::iterator_traits<Iterator>::difference_type d2,
        Function& f,
        typename std::iterator_traits<Iterator>::difference_type d = 0)
{
    ...
    else
    {
        ...
        {
            if (!combine_discontinuous_conditional(f1p, last1, d1-1, i2, last2, d22, f,
                                                   d + 1))
            {
                return false;
            }
...
inline void for_each_combination(Iterator begin, Iterator end,
                                 ...
                                 Function function)
{
    if (static_cast<typename std::iterator_traits<Iterator>::difference_type>
...
inline unsigned long long int n_choose_k(const unsigned long long int& n,
        const unsigned long long int& k)
{
    ...
    public:
        ...
        inline value_type& lookup(const value_type& n, const value_type& k)
        {
            ...
            return table_[static_cast<std::size_t>((dimension_ * n) + std::min<value_type>
                                                                                        (k,difference))];
        }
    ...
    static const std::size_t static_table_size = static_cast<std::size_t>((
                                     ...
                                     const bool complete_index = true)
{
    ...
    index_list[k - 1] = index_list[k - 2] + static_cast<std::size_t>
                                     ...
                                     const bool complete_index = true)
{
    ...
    nth_combination_sequence(n,length,k,std::back_inserter(index_list),
                             complete_index);
...
template <typename OutputIterator>
inline void nth_permutation_sequence(std::size_t n, const std::size_t k,
                                     OutputIterator out)
{
    //Note: n in [0,k!)
...
inline std::string nth_permutation_sequence(const std::size_t& n,
        const std::string& s)
{
...
public:
    ...
    explicit inline combination_iterator(const std::size_t& k,
                                         std::string& str,
                                         const bool sorted = true)
        : begin_(const_cast<char*>(to_ptr(str))),
          end_(const_cast<char*>(to_ptr(str) + str.size())),
          middle_(begin_ + k),
          current_combination_(begin_,middle_)
    {
        if (!sorted)
    ...
    inline combination_iterator(const std::string& str)
        : begin_(const_cast<char*>(to_ptr(str) + str.size())),
          end_(begin_),
          middle_(end_),
...
{
    return all_digits_check<N,const char*>(to_ptr(s));
}
...
inline bool all_digits_check(const std::string& s)
{
    return all_digits_check(s.size(), to_ptr(s));
}
...
{
    return signed_all_digits_check<N,const char*>(to_ptr(s));
}
...
inline bool signed_all_digits_check(const std::string& s)
{
    return signed_all_digits_check(s.size(), to_ptr(s));
}
...
inline void numeric_convert(const std::string& s, T& t,
                            const bool digit_check = false)
{
    numeric_convert<N,T,const char*>(to_ptr(s), t, digit_check);
}
...
template <typename T>
inline bool numeric_convert(const std::string& s, T& t,
                            const bool digit_check = false)
{
    return numeric_convert(s.size(), to_ptr(s), t, digit_check);
}

template <std::size_t N, typename T, typename Iterator>
inline bool signed_numeric_convert(Iterator itr, T& t,
                                   const bool digit_check = false)
{
    if ('-' == (*itr))
    {
...
inline bool signed_numeric_convert(const std::string& s,
                                   ...
                                   const bool digit_check = false)
{
    return signed_numeric_convert<N,T,const char*>(to_ptr(s), t, digit_check);
}
...
inline bool signed_numeric_convert(const std::string& s,
                                   ...
                                   const bool digit_check = false)
{
    return signed_numeric_convert<T,const char*>(to_ptr(s), to_ptr(s) + s.size(), t,
            digit_check);
}
...
static inline unsigned long long int convert_to_be(const unsigned long long int
        v)
{
    return (is_little_endian()) ? convert(v) : v;
}
...
static inline unsigned long long int convert_to_le(const unsigned long long int
        v)
{
    return (is_big_endian()) ? convert(v) : v;
}
...
public:
    ...
    template <typename T>
    inline bool operator()(T*& data, uint64_t& length,
                           const bool read_length = true)
    {
        uint32_t l = 0;
...
#undef strtk_def_rdr_opr
    ...
    inline bool operator()(T& output, const std::size_t& size)
    {
        ...
        {
            bool result = strtk::string_to_type_converter<char*,T>(buffer_,buffer_ + size,
                          output);
            buffer_ += size;
...
private:
    ...
    public:
        ...
        typedef selector_impl<Type,typename strtk::details::is_pod<Type>::result_t>
        type;
    };
...
public:
    ...
    {
        return operator()<const char>(to_ptr(input),
...
#undef strtk_def_wrtr_opr
    ...
    inline bool operator()(const std::multiset<T,Allocator,Comparator>& multiset)
    {
        ...
        typename std::multiset<T,Allocator,Comparator>::const_iterator itr =
            multiset.begin();
        typename std::multiset<T,Allocator,Comparator>::const_iterator end =
            multiset.end();
    ...
    {
        stream.write(original_buffer_,
                     static_cast<std::streamsize>(amount_written_sofar_));
        return amount_written_sofar_;
    }
                           ...
                           const char padding = ' ')
    {
        if (amount_written_sofar_ + size <= buffer_length_)
        {
            ...
            return operator()<const char>(to_ptr(s), static_cast<uint32_t>(size), false);
        }
        else
        {
            return false;
...
public:
    ...
    template <typename InputIterator>
    inline hex_to_number_sink& operator=(const
                                         std::pair<InputIterator,InputIterator>& s)
    {
        std::size_t offset = 0;
        const std::size_t size = std::distance(s.first,s.second);

        if ((size > 2) && ((*s.first) == '0') && (((*(s.first + 1)) == 'x') ||
                ((*(s.first + 1)) == 'X')))
        {
            offset = 2;
        }
    ...
    {
        return this->operator =(std::make_pair(to_ptr(s), to_ptr(s) + s.size()));
    }
...
public:
    ...
    {
        if (!range_only_contains(base64_value_check(), to_ptr(s), to_ptr(s) + s.size()))
        {
            valid_ = false;
            return (*this);
        }
        ...
        convert_base64_to_bin(to_ptr(s), to_ptr(s) + s.size(),
                              reinterpret_cast<char*>(t_));
        reverse_bytes();
    ...
    template <typename InputIterator>
    inline base64_to_number_sink& operator=(const
                                            std::pair<InputIterator,InputIterator>& s)
    {
...
public:
    ...
    template <typename InputIterator>
    inline hex_to_string_sink& operator=(const
                                         std::pair<InputIterator,InputIterator>& s)
    {
        const std::size_t size = std::distance(s.first,s.second);
        std::size_t offset = 0;
        ...
        valid_ = convert_hex_to_bin(s.first + offset,
                                    s.second,
                                    const_cast<char*>(to_ptr(s_)));
        return (*this);
    }

    inline hex_to_string_sink& operator=(const std::string& s)
    {
        return this->operator=(std::make_pair<char*>(const_cast<char*>(to_ptr(s)),
                               const_cast<char*>(to_ptr(s) + s.size())));
    }
...
public:
    ...
    inline bool operator()(InputIterator begin, InputIterator end)
    {
        ...
        if (std::numeric_limits<std::size_t>::max() != fractional_size_)
        {
            ...
            {
                return strtk::string_to_type_converter(begin,
                                                       begin + (size - (fractional_size_ + 1)),(*t_));
            }
        }
...
public:
    ...
    inline sink_type& count(const std::size_t& element_count =
        ...
        if (container_)
        {
            if (std::numeric_limits<std::size_t>::max() == element_count_)
                ...
                return (details::parse_n_stl_container_proxy
                        (begin, end, delimiters_, element_count_, (*container_),
                         split_option_) == element_count_);
        }
        else
        {
            return false;
...
template <typename T> struct vector_sink
{
    typedef sink_type<std::vector<T> >   type;
};
template <typename T> struct deque_sink
{
    typedef sink_type<std::deque<T> >    type;
};
template <typename T> struct list_sink
{
    typedef sink_type<std::list<T> >     type;
};
template <typename T> struct set_sink
{
    typedef sink_type<std::set<T> >      type;
};
template <typename T> struct multiset_sink
{
    typedef sink_type<std::multiset<T> > type;
};
template <typename T> struct queue_sink
{
    typedef sink_type<std::queue<T> >    type;
};
template <typename T> struct stack_sink
{
    typedef sink_type<std::stack<T> >    type;
};
template <typename T> struct priority_queue_sink
{
    typedef sink_type<std::priority_queue<T> > type;
};
...
inline void swap_inplace(std::string& s, const std::size_t& i0,
                         const std::size_t& i1)
{
    if (i0 >= s.size())
    {
...
inline std::string swap(const std::string& s, const std::size_t& i0,
                        const std::size_t& i1)
{
    std::string result = s;
...
inline void remove_inplace(std::string& s, const std::size_t& index)
{
    ...
    std::memcpy(const_cast<char*>(to_ptr(s) + index),
                const_cast<char*>(to_ptr(s) + (index + 1)), s.size() - index);
    s.resize(s.size() - 1);
}
...
inline void insert_inplace(std::string& s, const std::size_t& index,
                           const char c)
{
    s.resize(s.size() + 1);
    std::memcpy(const_cast<char*>(to_ptr(s) + index + 1),
                const_cast<char*>(to_ptr(s) + (index)), s.size() - index);
    s[index] = c;
}

inline std::string insert(const std::string& s, const std::size_t& index,
                          const char c)
{
    std::string result = s;
...
template <typename Iterator, typename Predicate>
inline typename range_type<Iterator>::type find_exactly_n_consecutive_values(
    const std::size_t n,
    Predicate p,
    Iterator itr,
    const Iterator end,
...
template <typename Iterator, typename Predicate>
inline typename range_type<Iterator>::type find_atleast_n_consecutive_values(
    const std::size_t n,
    Predicate p,
    Iterator itr,
    const Iterator end)
...
inline typename range_type<Iterator>::type find_exactly_n_consecutive_values(
    ...
    const bool stateful_predicate = false)
{
    return find_exactly_n_consecutive_values(n,p,range.first,range.second,
            stateful_predicate);
}
...
template <typename Iterator, typename Predicate>
inline typename range_type<Iterator>::type find_n_consecutive_values(
    const std::size_t n,
    find_mode::type mode,
    Predicate p,
    Iterator itr,
    const Iterator end)
{
    switch (mode)
    {
    case find_mode::exactly_n :
        return find_exactly_n_consecutive_values(n,p,itr,end);

    case find_mode::atleast_n :
        return find_atleast_n_consecutive_values(n,p,itr,end);
                                       ...
                                       const Iterator end)
{
    ...
    {
    case find_mode::exactly_n :
        return match_exactly_n_consecutive_values(n,p,itr,end);

    case find_mode::atleast_n :
        return match_atleast_n_consecutive_values(n,p,itr,end);
...
template <typename Iterator>
inline typename details::range_type<Iterator>::type find_n_consecutive(
    const std::size_t n,
    find_type::type type,
    find_mode::type mode,
    typename details::range_type<Iterator>::type range)
{
    switch (type)
    {
    case find_type::digits  :
        return details::find_n_consecutive_values<Iterator>(n,
                mode,
                strtk::text::is_digit,
                range.first,range.second);

    case find_type::letters :
        return details::find_n_consecutive_values<Iterator>(n,
                mode,
                strtk::text::is_letter,
                range.first,range.second);

    case find_type::lowercase_letters :
        return details::find_n_consecutive_values<Iterator>(n,
                mode,
                strtk::text::is_lowercase_letter,
                range.first,range.second);

    case find_type::uppercase_letters :
        return details::find_n_consecutive_values<Iterator>(n,
                mode,
                strtk::text::is_uppercase_letter,
                range.first,range.second);

    case find_type::letters_digits    :
        return details::find_n_consecutive_values<Iterator>(n,
                mode,
    ...
    default :
        return typename details::range_type<Iterator>::type(range.second,range.second);
    }
}
                                        ...
                                        OutputIterator out)
{
    ...
    {
        range_type found_itr = details::find_exactly_n_consecutive_values(m,p,
                               itr_range);
...
public:
    ...
    inline bool operator()(InputIterator begin, InputIterator end)
    {
        ...
        {
            return std::equal(to_ptr(s_), to_ptr(s_) + s_.size(), begin);
        }
    }
...
public:
    ...
    inline bool operator()(InputIterator begin, InputIterator end)
    {
        ...
        {
            return std::equal(to_ptr(s_), to_ptr(s_) + s_.size(), begin, imatch_char);
        }
    }
...
public:
    ...
    inline bool operator()(InputIterator begin, InputIterator end) const
    {
        ...
        return strtk::match(to_ptr(s_), to_ptr(s_) + s_.size(), begin, end,
...
struct convert_impl
{
                        ...
                        Type& t)
    {
        ...
        {
        case 0  :
            remove_leading_trailing(rem_chars,s);
            break;
...
struct convert_impl <std::string>
{
                        ...
                        std::string& t)
    {
        ...
        {
        case 0  :
            remove_leading_trailing(rem_chars,t);
            break;

        case 1  :
            remove_leading         (rem_chars,t);
            break;

        case 2  :
            remove_trailing        (rem_chars,t);
            break;
...
public:
    ...
    inline bool operator()(InputIterator begin, InputIterator end)
    {
        return trim_details::convert_impl<T>::execute(begin, end, rem_chars_, mode_,
                (*t_));
    }
...
public:
    ...
    inline bool operator()(InputIterator begin, InputIterator end)
    {
        const std::size_t range_size = static_cast<std::size_t>(std::distance(begin,
                                       end));
...
inline details::fill_array_impl fill_array(unsigned char* data,
        const std::size_t& size)
{
...
inline details::fill_array_impl fill_array(std::string& data,
        const std::size_t& size)
{
    return fill_array(const_cast<char*>(to_ptr(data)),size);
}

inline details::fill_array_impl fill_array(std::string& data)
{
    return fill_array(const_cast<char*>(to_ptr(data)),data.size());
}
...
static const std::size_t digit_table_size = sizeof(digit_table) / sizeof(
            unsigned char);
...
inline T pow10(T d, const int exponent)
{
    ...
    static const int fract10_size = static_cast<int>(sizeof(fract10) / sizeof(
                                        double));
...
template<> struct numeric<float>
{
    enum { min_exp =  -37, max_exp =  +38, precision = 10};
};
template<> struct numeric<double>
{
    enum { min_exp = -307, max_exp = +308, precision = 15};
};

template <typename ld, std::size_t size> struct ldt {};
template <> struct ldt <long double,     sizeof(double)>
{
    enum { i =  -308, a =  +308, p = 15};
}; // 64-bit
template <> struct ldt <long double,                 10>
{
    enum { i = -4931, a = +4931, p = 18};
}; // 80-bit
template <> struct ldt <long double,                 12>
{
    enum { i = -4931, a = +4931, p = 22};
}; // 96-bit
template <> struct ldt <long double, 2 * sizeof(double)>
{
    enum { i = -4931, a = +4931, p = 34};
}; //128-bit
...
template<> struct supported_conversion_to_type<hex_to_string_sink>
{
    typedef hex_string_type_tag type;
};
...
template<> struct supported_conversion_to_type<bool>
{
    typedef bool_type_tag type;
};
template<> struct supported_iterator_type<bool>
{
    enum { value = true };
};

template<> struct supported_conversion_to_type<std::string>
{
    typedef stdstring_type_tag type;
};
template<> struct supported_iterator_type<std::string>
{
    enum { value = true };
};

template<> struct supported_conversion_to_type<strtk::util::value>
{
    typedef value_type_tag type;
};
template<> struct supported_conversion_from_type<strtk::util::value>
{
    typedef value_type_tag type;
};
template<> struct supported_iterator_type<strtk::util::value>
{
    enum { value = true };
};

template<> struct supported_conversion_to_type<strtk::details::expect_impl>
{
    typedef expect_type_tag type;
};
template<> struct supported_iterator_type<strtk::details::expect_impl>
{
    enum { value = true };
};

template<> struct supported_conversion_to_type<strtk::details::iexpect_impl>
{
    typedef expect_type_tag type;
};
template<> struct supported_iterator_type<strtk::details::iexpect_impl>
{
    enum { value = true };
};

template<> struct supported_conversion_to_type<strtk::details::like_impl>
{
    typedef like_type_tag type;
};
template<> struct supported_iterator_type<strtk::details::like_impl>
{
    enum { value = true };
};

template<> struct supported_conversion_to_type<strtk::details::fill_array_impl>
{
    typedef fillchararray_type_tag type;
};
template<> struct supported_iterator_type<strtk::details::fill_array_impl>
{
    enum { value = true };
};

template<> struct
    supported_conversion_to_type<strtk::details::conv_to_lcase_impl>
{
    typedef lcase_type_tag type;
};
template<> struct
    supported_iterator_type<strtk::details::conv_to_lcase_impl>
{
    enum { value = true };
};

template<> struct
    supported_conversion_to_type<strtk::details::conv_to_ucase_impl>
{
    typedef ucase_type_tag type;
};
template<> struct
    supported_iterator_type<strtk::details::conv_to_ucase_impl>
{
    enum { value = true };
};
...
template<> struct supported_conversion_to_type<ignore_token>
{
    typedef ignore_token_type_tag type;
};
...
inline bool string_to_type_converter_impl(Iterator& begin, const Iterator end,
        T& t, not_supported_type_tag)
{
#ifdef strtk_enable_lexical_cast
...
template <typename Iterator>
inline bool string_to_type_converter_impl(Iterator& begin, const Iterator end,
        strtk::util::value& v, value_type_tag)
{
    return v(begin,end);
}
...
template <typename Iterator>
inline bool string_to_type_converter_impl(Iterator& begin, const Iterator end,
        std::string& t, stdstring_type_tag)
{
    t.assign(begin,end);
    begin = end;
    return true;
...
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
        Expect& t, expect_type_tag)
{
    if (!t(itr,end))
    {
        return false;
...
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
        Array& t, fillchararray_type_tag)
{
    if (!t(itr,end))
    {
        return false;
...
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
        TruncatedInt& t, truncint_type_tag)
{
    if (!t(itr,end))
    {
        return false;
...
template <typename Iterator, typename T>
inline bool string_to_type_converter_impl(Iterator& itr_external,
        const Iterator end, T& result, unsigned_type_tag)
{
    ...
    if (strtk_unlikely((length = std::distance(itr,
                                 end)) > numeric<T>::bound_length))
    {
        return false;
    }
...
#define case_stmt(N)                                      \
    ...
    case  1 :
        if (strtk_unlikely((digit = (*itr - '0'))>= 10))
        {
            return false;
        }
...
template <typename Iterator, typename T>
inline bool string_to_type_converter_impl(Iterator& itr_external,
        const Iterator end, T& result, signed_type_tag)
{
    Iterator itr;
...
template <typename Iterator, typename T>
inline bool string_to_type_converter_impl_ref(Iterator& itr, const Iterator end,
        T& result, signed_type_tag)
{
    if (itr == end)
    {
        return false;
...
inline bool parse_nan(Iterator& itr, const Iterator end, T& t)
{
    ...
    {
        if ((static_cast<type>('a') != *(itr + 1)) ||
                (static_cast<type>('n') != *(itr + 2)))
        {
            return false;
        }
    }
    else if ((static_cast<type>('A') != *(itr + 1)) ||
             (static_cast<type>('N') != *(itr + 2)))
    {
        return false;
    }
...
template <typename Iterator, typename T>
inline bool string_to_type_converter_impl(Iterator& itr_external,
        const Iterator end, T& t, real_type_tag)
{
    if (end == itr_external)
    {
        return false;
...
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
        IgnoreTokenType&, ignore_token_type_tag)
{
    itr = end;
    return true;
}
...
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
        HexSinkType& t, hex_number_type_tag)
{
...
template <typename Iterator, typename HexSinkType>
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
        HexSinkType& t, hex_string_type_tag)
{
    t = std::pair<Iterator,Iterator>(itr,end);
...
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
        Base64SinkType& t, base64_type_tag)
{
    t = std::pair<Iterator,Iterator>(itr,end);
...
template <typename T>
inline bool type_to_string_converter_impl(const T& t, std::string& s,
        not_supported_type_tag)
{
#ifdef strtk_enable_lexical_cast
...
template <typename T>
inline bool type_to_string_converter_impl(T value, std::string& result,
        unsigned_type_tag)
{
    ...
    static const std::size_t buffer_size = ((strtk::details::numeric<T>::size < 16)
                                            ? 16 : 32);
...
template <typename T>
inline bool type_to_string_converter_impl(T valuex, std::string& result,
        signed_type_tag)
{
    ...
    const std::size_t buffer_size = ((strtk::details::numeric<T>::size < 16) ? 16 :
                                     32);
...
template <typename T>
inline bool type_to_string_converter_impl(const T& value, std::string& result,
        byte_type_tag)
{
    result.resize(1);
...
inline bool type_to_string_converter_impl(const bool& value,
        std::string& result, bool_type_tag)
{
    result.resize(1);
    result[0] = value ? '1' : '0';
    return true;
...
inline bool type_to_string_converter_impl(const std::string& value,
        std::string& result, stdstring_type_tag)
{
    result = value;
    return true;
}
...
template <typename Iterator>
inline bool type_to_string_converter_impl(const std::pair<Iterator,Iterator>&
        range, std::string& result, stdstring_range_type_tag)
{
...
inline bool type_to_string_converter_impl(const SinkType&, std::string&,
        sink_type_tag)
{
...
inline bool type_to_string_converter_impl(const STLContainerType&, std::string&,
        stl_seq_type_tag)
{
...
public:
    ...
    inline ext_string& remove_leading(const std::string& removal_set)
    {
        ...
        {
            strtk::remove_leading(single_delimiter_predicate<std::string::value_type>
                                  (removal_set[0]),s_);
        }
        else
        {
    ...
    inline ext_string& remove_trailing(const std::string& removal_set)
    {
        ...
        {
            strtk::remove_trailing(single_delimiter_predicate<std::string::value_type>
                                   (removal_set[0]),s_);
        }
        else
        {
    ...
    template <typename T,
              typename Allocator,
              template <typename,typename> class Sequence>
    inline std::size_t parse(const std::string& delimiters,
                             Sequence<T,Allocator>& seq) const
    {
    ...
    template <typename T,
              typename Allocator,
              template <typename,typename> class Sequence>
    inline std::size_t parse(const char* delimiters,
                             Sequence<T,Allocator>& seq) const
    {
    ...
    friend inline ext_string operator - (const ext_string& s,
                                         const std::string& pattern);
    friend inline ext_string operator - (const ext_string& s, const char* pattern);
    friend inline ext_string operator - (const ext_string& s,
                                         const ext_string& pattern);
...
inline bool load_file(const std::string& file_name, char* buffer,
...
inline bool load_file(const std::string& file_name, std::string& buffer)
{
    ...
    return load_file(file_name,const_cast<char*>(to_ptr(buffer)),buffer.size());
}

inline bool write_file(const std::string& file_name, char* buffer,
                       const std::size_t& buffer_size)
{
...
{
    return write_file(file_name,const_cast<char*>(to_ptr(buffer)),buffer.size());
}

inline bool copy_file(const std::string& src_file_name,
                        ...
                        const std::string& output_file_name)
{
    ...
    while (round < 2)
    {
        ...
        remaining_bytes = ((0 == round) ? file_size(file_name1) : file_size(
                               file_name2));
...
inline bool files_identical(const std::string& file_name1,
                            const std::string& file_name2)
{
    std::ifstream file1(file_name1.c_str(),std::ios::binary);
    std::ifstream file2(file_name2.c_str(),std::ios::binary);
    ...
    if (file_size(file_name1) != file_size(file_name2))
    {
        return false;
    }
...
{
    return (false != stream.read(reinterpret_cast<char*>(&t[0]),
                                 sizeof(T) * N).fail());
}
...
{
    return (false != stream.write(reinterpret_cast<char*>(&t[0]),
                                  sizeof(T) * N).fail());
}
...
inline unsigned char* read_pod(unsigned char* data,
                               ...
                               T9& t9, T10& t10, T11& t11, T12& t12)
{
    ::memcpy(& t1, data, sizeof( T1));
    data += sizeof( T1);
    ::memcpy(& t2, data, sizeof( T2));
    data += sizeof( T2);
    ::memcpy(& t3, data, sizeof( T3));
    data += sizeof( T3);
    ::memcpy(& t4, data, sizeof( T4));
    data += sizeof( T4);
    ::memcpy(& t5, data, sizeof( T5));
    data += sizeof( T5);
    ::memcpy(& t6, data, sizeof( T6));
    data += sizeof( T6);
    ::memcpy(& t7, data, sizeof( T7));
    data += sizeof( T7);
    ::memcpy(& t8, data, sizeof( T8));
    data += sizeof( T8);
    ::memcpy(& t9, data, sizeof( T9));
    data += sizeof( T9);
    ::memcpy(&t10, data, sizeof(T10));
    data += sizeof(T10);
    ::memcpy(&t11, data, sizeof(T11));
    data += sizeof(T11);
    ::memcpy(&t12, data, sizeof(T12));
    data += sizeof(T12);
    return data;
}
                                ...
                                const T9& t9, const T10& t10, const T11& t11, const T12& t12)
{
    ...
    data += sizeof( T1);
    ::memcpy(data, &t2, sizeof( T2));
    data += sizeof( T2);
    ::memcpy(data, &t3, sizeof( T3));
    data += sizeof( T3);
    ::memcpy(data, &t4, sizeof( T4));
    data += sizeof( T4);
    ::memcpy(data, &t5, sizeof( T5));
    data += sizeof( T5);
    ::memcpy(data, &t6, sizeof( T6));
    data += sizeof( T6);
    ::memcpy(data, &t7, sizeof( T7));
    data += sizeof( T7);
    ::memcpy(data, &t8, sizeof( T8));
    data += sizeof( T8);
    ::memcpy(data, &t9, sizeof( T9));
    data += sizeof( T9);
    ::memcpy(data,&t10, sizeof(T10));
    data += sizeof(T10);
    ::memcpy(data,&t11, sizeof(T11));
    data += sizeof(T11);
    ::memcpy(data,&t12, sizeof(T12));
    data += sizeof(T12);
    return data;
}
...
private:
    ...
    inline bool condition_begins_with(const itr_type begin,
                                      const itr_type end) const
    {
        if (s.size() == static_cast<std::size_t>(std::distance(begin,end)))
        {
    ...
    typedef bool (string_condition::*condition_method)(const itr_type begin,
            const itr_type end) const;
...
public:
    ...
    inline explicit string_condition(condition_type cond_type,
                                     const std::string& str)
        : cond_type_(cond_type),
          s(str),
          s_begin(reinterpret_cast<const unsigned char*>(to_ptr(s))),
          s_end  (reinterpret_cast<const unsigned char*>(to_ptr(s) + str.size())),
          condition_method_(0)
    {
        switch (cond_type_)
        {
        case equal       :
            condition_method_ = &string_condition::condition_equal;
            break;

        case notequal    :
            condition_method_ = &string_condition::condition_notequal;
            break;

        case like        :
            condition_method_ = &string_condition::condition_like;
            break;

        case begins_with :
            condition_method_ = &string_condition::condition_begins_with;
            break;

        case ends_with   :
            condition_method_ = &string_condition::condition_ends_with;
            break;

        case within      :
            condition_method_ = &string_condition::condition_within;
            break;

        case notwithin   :
            condition_method_ = &string_condition::condition_notwithin;
            break;
        }
    }
    ...
    inline bool operator()(const std::string& str)
    {
        return operator()
               (
                   reinterpret_cast<const unsigned char*>(to_ptr(str)),
                   reinterpret_cast<const unsigned char*>(to_ptr(str) + str.size())
               );
    }
...
public:
    ...
    template <typename key_iterator_t>
    inline bool find_prefix(const key_iterator_t begin,
                            const key_iterator_t end) const
    {
        if ((0 == head_) || (0 == std::distance(begin,end)))
        {
            return false;
...
public:
    ...
    inline bool operator!()
    {
        return (minimum_size > maximum_size)      ||
               ...
               (std::numeric_limits<double>::infinity() == std::abs(
    ...
    virtual bool compute_optimal_parameters()
    {
        ...
        */
        ...
        while (k < 1000.0)
        {
            ...
            double denominator = std::log(1.0 - std::pow(false_positive_probability,
                                          1.0 / k));
...
public:
          ...
          desired_false_positive_probability_(p.false_positive_probability)
    {
        ...
        std::fill_n(&bit_table_[0], static_cast<std::size_t>(raw_table_size_),
    ...
    inline filter& operator = (const filter& f)
    {
        if (this != &f)
        {
            ...
            desired_false_positive_probability_ = f.desired_false_positive_probability_;
            delete[] bit_table_;
            bit_table_               = new cell_type[static_cast<std::size_t>
                    (raw_table_size_)];
    ...
    {
        insert(reinterpret_cast<const unsigned char*>(to_ptr(key)),key.size());
    }
    ...
    {
        return contains(reinterpret_cast<const unsigned char*>(to_ptr(key)),key.size());
    }
    ...
    inline double effective_fpp() const
    {
        ...
        */
        return std::pow(1.0 - std::exp(-1.0 * salt_.size() * inserted_element_count_ /
                                       size()), 1.0 * salt_.size());
    }
...
protected:
        ...
        if (salt_count_ <= predef_salt_count)
        {
            ...
            for (unsigned int i = 0; i < salt_.size(); ++i)
            {
                /*
                  ...
                  instances.
                */
                salt_[i] = salt_[i] * salt_[(i + 3) % salt_.size()] + static_cast<bloom_type>
                           (random_seed_);
            }
        }
        else
    ...
    inline bloom_type hash_ap(const unsigned char* begin,
                              std::size_t remaining_length, bloom_type hash) const
    {
        const unsigned char* itr = begin;
        unsigned int loop = 0;
...
public:
    ...
    inline bool compress(const double& percentage)
    {
        ...
        unsigned long long int new_table_size = static_cast<unsigned long long int>((
                size_list.back() * (1.0 - (percentage / 100.0))));
        new_table_size -= (((new_table_size % bits_per_char) != 0) ?
        ...
        cell_type* tmp = new cell_type[static_cast<std::size_t>(new_table_size /
                                                                               bits_per_char)];
...
inline void compute_pod_hash(const unsigned long long int& data,
                             unsigned int& hash)
{
    const unsigned char* itr = reinterpret_cast<const unsigned char*>(&data);
    hash ^=  ((hash <<  7) ^  itr[0] * (hash >> 3));
...
inline void compute_block(unsigned char* itr, std::size_t& length,
                          unsigned int& hash)
{
...
template <typename Iterator>
inline void hash(const Iterator itr, std::size_t length,
                 unsigned int& hash_value)
{
    if (length >= 64)
    {
        compute_block<64>(itr,length,hash_value);
    }

    if (length >= 32)
    {
        compute_block<32>(itr,length,hash_value);
    }

    if (length >= 16)
    {
        compute_block<16>(itr,length,hash_value);
    }

    if (length >=  8)
    {
        compute_block< 8>(itr,length,hash_value);
    }

    if (length >=  4)
    {
        compute_block< 4>(itr,length,hash_value);
    }

    if (length >=  2)
    {
        compute_block< 2>(itr,length,hash_value);
    }

    if (length ==  0)
    {
        compute_block< 1>(itr,length,hash_value);
    }
}
...
template <typename T,
          typename Allocator,
          template <typename,typename> class Sequence>
inline unsigned int hash(const Sequence<T,Allocator>& sequence,
                         unsigned int seed = details::hash_seed)
{
    unsigned int hash_value = seed;
...
public:
    ...
    semantic_action_impl()
        : function_holder_(0)
    {
        std::fill_n(function_holder_buffer_, sizeof(function_holder_buffer_),
                    static_cast<unsigned char>(0x00));
    }
    ...
    {
        return operator()(to_ptr(s), to_ptr(s) + s.size());
    }
    ...
    inline void assign(Function& f)
    {
        ...
        function_holder_ = construct<Function,
        type_size <= function_holder_buffer_size>::type(f,function_holder_buffer_);
    }
...
template <typename Iterator, typename T>
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
        strtk::util::attribute<T>& result, attribute_type_tag)
{
...
template <typename T>
inline bool type_to_string_converter_impl(const strtk::util::attribute<T>&
        attrib, std::string& result, attribute_type_tag)
{
...
template<> struct supported_conversion_to_type
    < strtk::util::semantic_action_impl >
{
    typedef semantic_action_type_tag type;
};
template<> struct
    supported_conversion_from_type< strtk::util::semantic_action_impl >
{
    typedef semantic_action_type_tag type;
};
...
template <typename Iterator>
inline bool string_to_type_converter_impl(Iterator& itr, const Iterator end,
        strtk::util::semantic_action_impl& result, semantic_action_type_tag)
{
    return result(itr,end);
}

inline bool type_to_string_converter_impl(const
        strtk::util::semantic_action_impl&, std::string& result,
        semantic_action_type_tag)
{
...
public:
    ...
    value()
        : type_holder_(0)
    {
        std::fill_n(type_holder_buffer_, sizeof(type_holder_buffer_),
                    static_cast<unsigned char>(0x00));
    }
    ...
    inline void assign(T& t)
    {
        ...
        type_holder_ = construct<T,type_size <= type_holder_buffer_size>::type(t,
                       type_holder_buffer_);
    }
...
template <typename Key,
          ...
          typename OutputIterator>
inline void make_key_list_impl(const std::map<Key,T,Comparator,MapAllocator>&
                               map,
                               OutputIterator out)
{
    if (map.empty())
...
template <typename Key,
          ...
          typename MapAllocator,
          typename OutputIterator>
inline void make_value_list(const std::multimap<Key,T,Comparator,MapAllocator>&
                            map,
                            const Key& key,
                            OutputIterator out)
{
...
{
    typename std::multimap<Key,T*,Comparator,Allocator>::iterator itr =
        cont.begin();
                      ...
                      const T& v9, const T& v10, const T& v11)
{
    ...
    sequence.push_back( v6);
    sequence.push_back( v7);
    sequence.push_back( v8);
    sequence.push_back( v9);
    sequence.push_back(v10);
...
public:
    ...
    typedef column_selector_base<column_selector_impl<T0,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11>,12>
    csb_t;
        ...
        : csb_t(column_list)
    {
        ...
        csb_t::cvl().register_value( t1);
        csb_t::cvl().register_value( t2);
        csb_t::cvl().register_value( t3);
        csb_t::cvl().register_value( t4);
        csb_t::cvl().register_value( t5);
        csb_t::cvl().register_value( t6);
        csb_t::cvl().register_value( t7);
        csb_t::cvl().register_value( t8);
        csb_t::cvl().register_value( t9);
        csb_t::cvl().register_value(t10);
        csb_t::cvl().register_value(t11);
    }
};
...
public:
    ...
    typedef column_selector_base<column_selector_impl<T0,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10>,11>
    csb_t;
...
public:
    ...
    typedef column_selector_base<column_selector_impl<T0,T1,T2,T3,T4,T5,T6,T7,T8,T9>,10>
    csb_t;
...
public:
    ...
    typedef column_selector_base<column_selector_impl<T0,T1,T2,T3,T4,T5,T6,T7,T8>,9>
    csb_t;
...
public:
    ...
    typedef column_selector_base<column_selector_impl<T0,T1,T2,T3,T4,T5,T6,T7>,8>
    csb_t;
...
public:
    ...
    typedef column_selector_base<column_selector_impl<T0,T1,T2,T3,T4,T5,T6>,7>
    csb_t;
            ...
            const std::size_t& idx10, const std::size_t& idx11)
{
    ...
    cli.index_list[ 1] = idx1;
    cli.index_list[ 2] =  idx2;
    cli.index_list[ 3] = idx3;
    cli.index_list[ 4] =  idx4;
    cli.index_list[ 5] = idx5;
    cli.index_list[ 6] =  idx6;
    cli.index_list[ 7] = idx7;
    cli.index_list[ 8] =  idx8;
    cli.index_list[ 9] = idx9;
    cli.index_list[10] = idx10;
    cli.index_list[11] = idx11;
    return cli;
}
...
struct compose_st_selector_impl <T,1>
{
    ...
    template <typename Allocator,
              template <typename,typename> class Sequence>
    static inline type create(const column_list_t& col_list,
                              Sequence<T,Allocator>& seq)
    {
...
struct compose_st_selector_impl <T,10>
{
                              ...
                              std::list<T,Allocator>& list)
    {
        typename std::list<T,Allocator>::iterator b = list.begin();
        return type(col_list,*(b),*inc(b,1),*inc(b,2),*inc(b,3),*inc(b,4),*inc(b,5),
                    *inc(b,6),*inc(b,7),*inc(b,8),*inc(b,9));
    }
};
...
column_selector(const details::column_list_impl<N>& col_list,
                Sequence<T,Allocator>& seq)
{
    if (seq.size() >= N)
    {
...
public:
    ...
    column_selector_iterator_impl(const details::column_list_impl<N>& column_list,
...
#define strk_parse_col_seq \
      return parse_columns(to_ptr(data), to_ptr(data) + data.size(), delimiters, column_list, seq);
...
#define strtk_parse_col_begin()                                   \
      return parse_columns(to_ptr(data), to_ptr(data) + data.size(), \
                           delimiters,                               \
                           column_list,                              \
...
{
    return (*reinterpret_cast<T>(c1)) == (*reinterpret_cast<T>(c2));
}

template <std::size_t K>
struct size_impl
{
    static inline bool cmp(ptr,ptr)
    {
        return true;
    }
};
...
struct size_impl<8>
{
    static inline bool cmp(ptr c1, ptr c2)
    {
...
struct memcmp_n_impl
{
    static inline bool process(details::ptr c1, details::ptr c2)
    {
        ...
        return details::size_impl<size>::cmp(c1,c2) &&
               memcmp_n_impl<N - size>::process(c1 + size, c2 + size);
    }

    static inline bool process(const char* c1, const char* c2)
    {
        return memcmp_n_impl<N>::process(reinterpret_cast<details::ptr>(c1),
                                         reinterpret_cast<details::ptr>(c2));
    }

    template <std::size_t K1, std::size_t K2>
    static inline bool process(const unsigned char (&c1)[K1],
...
template<> struct memcmp_n_impl<0>
{
    static inline bool process(ptr,ptr)
    {
        return true;
    }
};
...
inline bool memcmp_n(const unsigned char (&c1)[K1],
                     const unsigned char (&c2)[K2])
{
    return details::memcmp_n_impl<N>::process(c1,c2);
}
...
namespace details
{
inline bool type_to_string_converter_impl(const strtk::util::value& v,
        std::string& result, value_type_tag)
{
...
inline std::pair<Iterator1,Iterator2> make_pair(const std::string& s)
{
    return std::make_pair<Iterator1,Iterator2>(
               reinterpret_cast<Iterator1>(const_cast<char*>(to_ptr(s))),
               reinterpret_cast<Iterator2>(const_cast<char*>(to_ptr(s) + s.size())));
}

template <typename Iterator1, typename Iterator2>
inline std::pair<Iterator1,Iterator2> make_pair(const
...
template <typename Iterator>
inline std::pair<Iterator,Iterator> make_pair(const
        std::pair<const char*, const char*>& p)
{
...
template <typename Iterator1, typename Iterator2>
inline std::pair<Iterator1,Iterator2> make_pair(const strtk::range::string&
        range)
{
    return std::make_pair<Iterator1,Iterator2>(
...
template <std::size_t N>
inline std::string make_string(const unsigned char (&s)[N],
                      ...
                      const unsigned char padding = '0')
{
    ...
    std::copy(to_ptr(s), to_ptr(s) + s.size(), &a[0]);
...
{
    std::fill(const_cast<char*>(to_ptr(s)),const_cast<char*>(to_ptr(s) + s.size()),
              v);
}
...
template <typename T>
inline void fill(const range::adapter<const T>& r,
                 const typename range::adapter<const T>::value_type& v)
{
...
inline void fill(const std_string::iterator_type& range,
                 const std::string::value_type& v)
{
    char* begin = const_cast<char*>(range.first);
    char* end = const_cast<char*>(range.second);
    std::fill(begin,end,v);
...
struct keygen<Range,unsigned int>
{
    static inline unsigned int transform(const Range& key_range)
    {
        ...
        if (strtk::fast::numeric_convert(std::distance(key_range.first,
                                         key_range.second),key_range.first,result,true))
        {
            return result;
        }
        else
...
public:
    ...
    inline bool operator()(const Range& key_range, const Range& value_range)
    {
        ...
        typename map_type::iterator itr = value_map_.find(
                                              details::keygen<Range,key_type>::transform(key_range));
...
static inline std::ostream& operator<<(std::ostream& os,
                                       const strtk::std_string::tokenizer<strtk::single_delimiter_predicate<char> >::type::iterator&
                                       range)
{
...
static inline std::ostream& operator<<(std::ostream& os,
                                       const strtk::std_string::tokenizer<strtk::single_delimiter_predicate<unsigned char> >::type::iterator&
                                       range)
{
    os << std::string((*range).first,(*range).second);
    return os;
...
static inline std::ostream& operator<<(std::ostream& os,
                                       const strtk::std_string::tokenizer<strtk::multiple_char_delimiter_predicate>::type::iterator&
                                       range)
{
    os << std::string((*range).first,(*range).second);
    return os;
		...
		public:
			...
			inline double time() const {
				if (!in_use_) {
					...
					return duration.count() * 0.000001; // 转换为秒
				}
				return std::numeric_limits<double>::max(); // 如果计时器仍在运行，返回最大值表示错误状态
			}
