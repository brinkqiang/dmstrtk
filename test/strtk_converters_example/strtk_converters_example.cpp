...
void example03()
{
    ...
    for (std::size_t k = 1; k < size; ++k)
    {
        ...
        {
            buffer1[i] = static_cast<unsigned char>(i);
        }

        std::size_t b64_size = strtk::convert_bin_to_base64(buffer1,buffer1 + k,
                               b64_buffer);
        ...
        for (std::size_t i = 0; i < k; ++i)
        {
            ...
            {
                std::cout << "example03 - failed bin-b64 conversions, error at position: " << i
                          << std::endl;
                return;
            }
        }
