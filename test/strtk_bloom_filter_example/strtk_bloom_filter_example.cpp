...
int main()
{
    ...
    static const std::size_t element_count = 2 * static_cast<std::size_t>
            (strtk::n_choose_k(n,k));
...

    {
        ...
        while (buf_end != buf_itr)
        {
            ...
            {
                std::cout << "Error: Failed to find: " << std::string(buf_itr,
                          buf_itr + k) << std::endl;
            }
...

    {
        ...
        {
            std::cout << "Error - Persisted filter and original filter do not match!" <<
                      std::endl;
            return 1;
        }
        else
        {
            ...
            while (buf_end != buf_itr)
            {
                ...
                {
                    std::cout << "Error: Failed to find: " << std::string(buf_itr,
                              buf_itr + k) << " in secondary filter!" << std::endl;
                    ++failures;
                }
