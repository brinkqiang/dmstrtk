...
void compute_stats(Sequence<double,Allocator>& value_list)
{
    ...
    std::nth_element(value_list.begin(),
                     value_list.begin() + (value_list.size() / 2), value_list.end());
...
int main(int argc, char* argv[])
{
    ...
    {
    case 1  :
        strtk::load_from_text_file(std::cin,str_list);
        break;

    case 2  :
        strtk::load_from_text_file(argv[1],str_list);
        break;
    ...
    for (std::size_t i = 0; i < str_list.size();
            total_length += str_list[i++].size()) ;
