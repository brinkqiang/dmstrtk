...
public:
    ...
    bool operator()(const std::string s, long long& period)
    {
        ...
        return operator ()(const_cast<char*>(s.data()),
                           const_cast<char*>(s.data() + s.size()), period);
    }

    template<typename Iterator>
    bool operator()(Iterator begin, Iterator end, long long& period)
    {
        static const strtk::split_options::type split_options =
            strtk::split_options::compress_delimiters +
            strtk::split_options::include_1st_delimiter;
        std::size_t token_count = split_n(predicate_, begin, end, max_token_count,
                                          token_list, split_options);
        ...
        {
            if (!strtk::string_to_type_converter(token_list[i].first,
                                                 token_list[i].second - 1, t))
            {
                return false;
            }
...
int main()
{
    ...
    const std::size_t period_string_size = sizeof(period_string) / sizeof(
            std::string);
