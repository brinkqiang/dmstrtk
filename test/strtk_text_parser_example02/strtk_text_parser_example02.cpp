...
const std::size_t not_of_interest_list_size = sizeof(not_of_interest_list) /
        sizeof(std::string);
...
public:
    ...
    typedef typename
    strtk::filter_on_match< strtk::range_to_type_back_inserter_iterator<Container> >
    filter_type;
    ...
    parse_line(Container& c, Predicate& p)
        : c_(c),
          ...
          filter_(reinterpret_cast<const std::string*>(not_of_interest_list),
                  reinterpret_cast<const std::string*>(not_of_interest_list +
                          not_of_interest_list_size),
